...
其中，$`I[k]`$ 表示第 $`k`$ 个频点的实部，$`Q[k]`$ 表示第 $`k`$ 个频点的虚部。由于复数频谱不能直接表示频点上的信号强弱，因此需要对其实部和虚部进行模值计算，得到该频点的幅度值：
...
在 FPGA 中，如果直接按照上述公式实现，需要完成平方、加法和开方运算，其中开方运算会消耗较多逻辑资源并引入较大的计算延迟。为了降低硬件实现复杂度，本文采用 αMax + βMin 近似求模算法对幅度值进行计算。该算法首先对实部和虚部取绝对值，并得到二者中的较大值和较小值：
...
$$
|X[k]| \approx \frac{15}{16}Max + \frac{15}{32}Min = \left(Max - (Max \gg 4)\right) + \left((Min \gg 1) - (Min \gg 5)\right)
$$

该形式中的系数可以通过移位和加减法实现，避免了平方和开方运算，更适合 FPGA 中的流水线处理。虽然该方法得到的是近似幅度值，但其计算结果能够较好反映各频点幅度的相对大小，满足频谱显示、信号检测和门限判断等应用需求。
...
![FFT Implementation](./Images/FFT_Impelentation.png)
...
4. 可以看到仿真中计算得到的幅度谱的频谱分布与MATLAB中画的基本一样，其频谱分布在 -20 MHz 到 +20 MHz 范围内，说明幅度谱计算模块的正确性。